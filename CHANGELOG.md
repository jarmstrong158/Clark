...
- **v2.11 — task-flow ramp + minimum-dwell mask + `per_task_switch` penalty.** A per-worker timeline audit found the policy thrashing — ~29 task switches/worker/day, operationally unusable though grade-neutral. Soft fix first: a `ticks_on_task` flow ramp (`TASK_FLOW_RAMP = (0.85, 0.92, 1.0, 1.03, 1.05)`, indexed on consecutive 10-min ticks; 0.85× setup floor on a fresh switch → 1.05× at ~40 min; `management`/`idle`/`cycle_count`/`off` exempt) makes sustained work genuinely faster via throughput, no new reward term to game. It wasn't enough — a flow-only retrain (`clark_foundation_flow.pt`, 500 ep) still showed ~29 switches/day; the throughput signal is too diffuse to beat the entropy bonus, same shape as the v2.5 filler attractor. So, structural (the v2.5 lesson): a minimum-dwell mask locks a worker to its current task for `DWELL_MIN_TICKS = 3` (30 min) before a non-emergency switch — the mask row collapses to the current task while `ticks_on_task < 3` and that task stays valid. Emergencies override by construction (OT / EOD / absent / pack-only branches `continue` past the lock; a task that just became invalid — daily-hours cap, restock filled, pick buffer full, stress-gated filler — releases it). Plus `per_task_switch = -1.5` on genuine churn (real → different-real only; idle starts/exits and forced redirects exempt). Dwell-floor off-by-one fixed: the counter increments *after* assignment, so the mask enforced only 2 ticks (20 min) until corrected to the intended 3. Dwell retrain (`clark_foundation_dwell.pt`, 500 ep, mask + penalty active) completed cleanly — win rates 70–85%, completion ~99% — continuity made structural at no throughput cost.
